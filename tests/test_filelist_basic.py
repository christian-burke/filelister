...
class TestAdd:
    ...
    def test_filelist_add_rel_string(self):
        flist = fs.Filelist(sample_data)
        flist3 = flist + str(
            os.path.relpath(rel_to_abs("data/sample_04.txt"), os.getcwd())
        )
        check_list = test_data.copy()
        check_list.append(rel_to_abs("data/sample_04.txt"))
        assert flist3.data == check_list
    ...
    def test_filelist_add_bad_string(self):
        with pytest.raises(FileNotFoundError, match="File Not Found: hello world!"):
            flist = fs.Filelist(sample_data)
            flist + "hello world!"
...
class TestIadd:
    ...
    def test_iadd_rel_string(self):
        flist = fs.Filelist(sample_data)
        flist += str(
            os.path.relpath(rel_to_abs("data/sample_04.txt"), start=os.getcwd())
        )
        test_list = sample_data.copy()
        test_list.append(rel_to_abs("data/sample_04.txt"))
        assert flist.data == test_list
...
class TestIsub:
    ...
    def test_isub_rel_string(self):
        flist = fs.Filelist(sample_data)
        flist -= str(
            os.path.relpath(rel_to_abs("data/sample_03.txt"), start=os.getcwd())
        )
        assert flist.data == sample_data[:2]
...
class TestSetCompars:
    ...
    def test_symmetric_difference(self):
        flist = fs.Filelist(sample_data)
        assert flist.symmetric_difference(sample_data2) == {
            test_data[0],
            test_data[1],
            test_data2[1],
            test_data2[2],
